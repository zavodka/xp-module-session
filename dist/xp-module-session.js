...
angular.module('xp-module-session').controller('SignInCtrl', function($auth, $scope, moduleSession, $q, xpFormHelper, $rootScope, customParams) {
  ...
  $scope.login = function() {
    ...
    if ($scope.signIn.$valid && !xpFormHelper.submitInProgress) {
      ...
      return $scope.loginPromise.then((function(data) {
        $auth.auth({
          name: data.username,
          roles: ['user']
        });
        ...
        return xpFormHelper.finishSubmiting();
      }), function(res) {
...
$templateCache.put("ui/loader/loader.html","<div class=\"inline-spinner\"></div>");}]);